...
void StatePlay::Draw(StateManager* sManager)
{
    ...
    //
    // View Hand/Model Rendering
    // TODO: Fix Items and Hands, Block is fine.
    //
	if(mWorld->mainOptions.guiDrawing == 1 && mWorld->HP > 0)
	{
        ...
        if(sleepTime <= 0.0f)
        {
            ...
            if(mWorld->invId[27+barPosition] == -1)
            {
				...
				// Set View Matrix as Identity
                sceGumMatrixMode(GU_VIEW);
                sceGumLoadIdentity();

                // Set the Model Origin
                ScePspFVector3 origin = {0.2f, -0.35f, -0.4f};
                //ScePspFVector3 origin = {0.5f + (cubeBob + shift_x) + sinf(animDest), -0.45f + (cubeBob2 + shift_y + changeY) + sinf(animDest), -0.725f};
                sceGumTranslate(&origin);
